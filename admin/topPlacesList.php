...
$topPlaces = $db->in_array("SELECT * FROM top_places  ORDER BY id ASC LIMIT $page_start, $page_count");
    
include "head.php";

$breadcump_title_1 = "Go'zal joylar";
$breadcump_title_2 = "Joylar ro'yxati";
?>
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                            ...
                            <table class="table table-responsive-md mb-0 table-bordered" id="table">
                                ...
                                <tbody id="customers">
                                    <? foreach ($topPlaces as $topPlace){ ?>
                                        <?
                                            $package = $db->assoc("SELECT * FROM packages WHERE id = ?", [ $topPlace["package_id"] ]);
                                            $image = image($package["image_id"]);
                                        ?>
                                        
                                        <tr class="btn-reveal-trigger">
                                            <td class="py-2"><?=$topPlace["id"]?></td>
                                            <td id="topPlace_image" class="py-2"><img src="<?=$image["file_folder"]?>" width="125px"></td>
                                            <td class="py-2"><?=$package["name"]?></td>
                                            <td class="py-2"><?=$package["days"]?></td>
                                            <td class="py-2"><?=$package["users"]?></td>
                                            <td class="py-2"><?=$package["location"]?></td>
                                            <td class="py-2"><?=$topPlace["created_date"]?></td>
                                            <td class="py-2 text-end">
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-right border py-0">
                                                        <div class="py-2">
                                                            <a class="dropdown-item"  href="/admin/editTopPlace/?id=<?=$topPlace["id"]?>">Tahrirlash</a>
                                                            <a class="dropdown-item text-danger" href="admin/editTopPlace/?id=<?=$topPlace["id"]?>&type=deletetopplace">O'chirish</a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </td>
                        ...
                        // count
                            $count = (int)$db->assoc("SELECT COUNT(*) FROM top_places")["COUNT(*)"];
