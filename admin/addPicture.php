...
if ($_REQUEST["type"] == $url[1]){
    ...
    $uploadedPictureImage = uploadImage("image", "images/pictures", ["jpg","jpeg","png"]);

    if (!$errors["forms"] || count($errors["forms"]) == 0) {
        $teacher_id = $db->insert("pictures", [
            "creator_user_id" => $user_id,
            "image_id" => $uploadedPictureImage["image_id"],
        ]);

        header("Location: picturesList/?page=1");
    } else {
...
$breadcump_title_2 = "yangi rasm qo'shish";
$form_title = "Yangi rasm qo'shish";
?>
